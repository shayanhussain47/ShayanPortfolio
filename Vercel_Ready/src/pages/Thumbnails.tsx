...
import { ArrowLeft, X, Palette, PenTool, Image as ImageIcon } from "lucide-react";
import GlobalLightLeaks from "@/components/GlobalLightLeaks";
import BloomDivider from "@/components/BloomDivider";
import CursorFollower from "@/components/CursorFollower";

// Dynamically load all images from the public/thumbs directory
const thumbFiles = import.meta.glob('/public/thumbs/*.{png,jpg,jpeg,webp}', { eager: true });

// Extract just the filenames to construct the correct public URL paths
const thumbnails = Object.keys(thumbFiles).map((path) => {
  return path.replace('/public/thumbs/', '');
});
        ...
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="mb-16">
            ...
            <p className="text-zinc-400 font-body text-sm md:text-base max-w-2xl">
              High-conversion cinematic thumbnails designed to maximize click-through rates.
            </p>
          </div>
            ...
            {thumbnails.map((img, index) => (
              ...
              >
                <div 
                  ...
                  onClick={() => setSelectedImage(`/thumbs/${img}`)}
                >
                  <img
                    src={`/thumbs/${img}`}
